...
#BOLA MEMANTUL DALAM RUANG KENTAL - MENGANDUNG GAYA GESEK
...
rho = 1.3 #Densitas medium
A = bola.radius*0.5 #cross sectional area
Cd = 0.25 #Koefisien hambatan

#Menghitung gaya gesek medium
Fd_x = - 0.5*rho*bola.velocity.x**2*A*Cd
Fd_y = - 0.5*rho*bola.velocity.y**2*A*Cd
Fd_z = - 0.5*rho*bola.velocity.z**2*A*Cd
Fd = vec(Fd_x, Fd_y, Fd_z)
...
while t < 30:
    ...
    #Update posisi dan kecepatan bola
                                        ...
                                        #m.a = m.g + Fd
                                        # a = (mg + Fd)/m
    bola.velocity = bola.velocity + (g*bola.mass+Fd/bola.mass) * dt
